...
env.generate_file(times,name='IQL')
...
for n in range(N):
    ...
    for num,time in enumerate(times):
        ...
        env.filedir = env.inp_train_file.format(iqll.name,num)
        observs,next_observs,reward,actions,_ = env.run_simulation(iqll)
        iqll.update_memory(observs,next_observs,reward,actions,env.update_num)
    ...
    iqll._epsilon_update()
    episode_reward = sum(rewards)
    episode_reward_history.append(episode_reward)

    test_reward,_,test_loss = env.test(iqll,test_time)
...
iqll.save()
np.save(iqll.model_dir+'train_loss_history.npy',np.array(train_loss_history))
np.save(iqll.model_dir+'test_loss_history.npy',np.array(test_loss_history))
np.save(iqll.model_dir+'episode_reward_history.npy',np.array(episode_reward_history))
np.save(iqll.model_dir+'test_reward_history.npy',np.array(test_reward_history))
...
fig,((axL,axP),(axM,axR)) = plt.subplots(nrows=2,ncols=2,figsize = (10,10))
# for idx in range(iqll.n_agents):
#     axL.plot(arange(len(episode_reward_history)),np.array(episode_reward_history)[:,idx],label = 'Agent %s'%idx)
...
axM.plot(arange(len(test_reward_history)),test_reward_history,label = 'score')
