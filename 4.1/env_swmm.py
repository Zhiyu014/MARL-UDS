...
class Ast:
    ...
    def run_simulation(self,agents,train=True,sensi = None):
        observs = []
        actions = []
        rewards = []
        # states = []
        ...
        with Simulation(inp_file) as sim:
            ...
            cum_flooding = 0
            cum_outfall = 0
            # cum_tank_inflow = [0 for _ in range(len(self.tanks))]
            # cum_tank_outflow = [0 for _ in range(len(self.tanks))]
            for st in sim:
                ...
                # tank_inflow = [nodes[tank].total_inflow - cum_tank_inflow[k] for k,tank in enumerate(self.tanks)]
                # tank_outflow = [nodes[tank].total_outflow - cum_tank_outflow[k] for k,tank in enumerate(self.tanks)]
                # cum_tank_inflow = [cum_tank_inflow[k] + tank_inflow[k] for k in range(len(self.tanks))]
                # cum_tank_outflow = [cum_tank_outflow[k] + tank_outflow[k] for k in range(len(self.tanks))]
                ...
                # reward = -flooding/inflow+outfall/inflow      #Reward 1
                reward = -flooding/inflow       #Reward 2
                    
                rewards.append(reward)    # rewards  (1,timesteps)
                actions.append(action)      # actions (action_size,n_agents,timesteps)
                
        #with read_out_file(self.BC_inp_file.replace('.inp','.out')) as out:
        #    bc_flooding = out.get_part('system')['Flow_lost_to_flooding']
        #with read_out_file(inp_file.replace('.inp','.out')) as out:
        #    sim_flooding = out.get_part('system')['Flow_lost_to_flooding']
        #    inflow = out.get_part('system')['Total_lateral_inflow']
        #rewards = (bc_flooding-sim_flooding)/inflow     #Reward 3
        ...
        observs,rewards,actions = observs[:-1],rewards[1:],actions[:-1]
        return observs,next_observs,rewards,actions,cum_inflow
    ...
    def test(self,f,rain,test_inp_file = None,sensi = None):
        ...
        observs,next_observs,rewards,actions,cum_inflow = self.run_simulation(f.agents,train=False,sensi=sensi)
    ...
    def test_file(self,file,label):
        rpt_path,_ = swmm5_run(file)
        reward = self.read_rpt(rpt_path)
        print(label+' Reward:   %s'%reward)
        return reward
