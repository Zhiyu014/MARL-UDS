...
class Ast:
    def __init__(self,inp_file = './Astlingen.inp',
                 inp_train_file = './train/Real_VDN_{}.inp',
                 inp_test_file = './test/Real_VDN_{}.inp',
        ...
        self.BC_setting = [0.2366,0.6508,0.3523,0.4303]
    ...
    # @ todo split files
    def generate_file(self,times,train = True):
        ...
        for idx,(start,end) in enumerate(times):
            filedir = self.inp_train_file.format(idx) if train else self.filedir
    ...
    def run_simulation(self,agents,
                       start = None, end = None, 
                       train=True, sensi = None, skip_dry = True):
        observs = []
        actions = []
        rewards = []
#        is_rain = False
...
#        dry_per = 0
    ...
    def test(self,f,time,sensi=None,filedir=None):
        ...
        # self.generate_file(date,train=False)
        start_time = datetime.strptime(time[0],'%m/%d/%Y %H:%M:%S')
        
        if filedir == None:            
            self.filedir = self.inp_test_file.format(str(start_time.date())+'-'+str(start_time.hour))
        else:
            self.filedir = filedir
        if exists(self.filedir) == False:
            self.generate_file([time],train=False)
        ...
        return score,act,loss
    
    def test_dura(self,f,start,end):
        self.filedir = self.inp_file
        observs,next_observs,rewards,actions,cum_inflow = self.run_simulation(f.agents,start = start,end = end, train=False)
