...
env.generate_file(times,name='DQN')
...
for n in range(N):
    ...
    for num,time in enumerate(times):
        ...
        observs,next_observs,reward,actions,_ = env.run_simulation(dqn)
        dqn.update_memory(observs,next_observs,reward,actions,env.update_num)
    ...
    if n>1:
        for _ in range(update_times):
            train_loss = dqn._experience_replay(batch_size)
    ...
    test_reward,acts,test_loss = env.test(dqn,test_time)
...
dqn.save()
np.save(dqn.model_dir+'train_loss_history.npy',np.array(train_loss_history))
np.save(dqn.model_dir+'test_loss_history.npy',np.array(test_loss_history))
np.save(dqn.model_dir+'episode_reward_history.npy',np.array(episode_reward_history))
np.save(dqn.model_dir+'test_reward_history.npy',np.array(test_reward_history))
